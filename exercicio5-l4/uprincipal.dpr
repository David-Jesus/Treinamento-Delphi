...
//    Writeln('O ano informado contém ' + IntToStr(31 mod 4));
//    Writeln('Dia mês ' + Semana[DayOfTheWeek(Now)]);
//    Writeln('Dia mês ' + Semana[DayOfTheWeek(EncodeDateTime(2010, 1, 12, 9, 10, 15, 0))]);
//    Writeln('Dia mês ' + IntToStr(DayOf(EncodeDateTime(2010, 1, 12, 9, 10, 15, 0))));
    while wDataInicial <= wDataFinal do
      begin
        while wMes <= 12 do
          begin
//            Writeln('Mês ' + IntToStr(wMes) + ' ' + Semana[DayOfTheWeek(EncodeDateTime(YearOf(wDataInicial), wMes, wDia, 0, 0, 0, 0))]);
...
//    while wMes <= 12 do
//      begin
//        Writeln('Mês ' + IntToStr(wMes) + ' ' + Semana[DayOfTheWeek(EncodeDateTime(2020, wMes, wDia, 0, 0, 0, 0))]);
//        wMes := wMes + 1;
//      end;
//      Writeln('Dat 1: ' + DateToStr(wDataInicial) + 'Data 2: ' + DateToStr(wDataFinal));
    Readln; //serve para deixa o console aberto até que seja clicado qualquer tecla
  except
    on E: Exception do
